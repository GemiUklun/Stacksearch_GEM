...
def check_cross(REF_period_ms,COMP_period_ms,fourier_bin,numharm,BIN_FACTOR):
        ...
        REF_period_low_ms = REF_period_low_s * 1000
        REF_period_high_ms = REF_period_high_s * 1000
       
        if (COMP_period_ms > REF_period_low_ms) and (COMP_period_ms < REF_period_high_ms):
                Curr_harm_str = "1/1"
                ...
                for nh in range(1, numharm + 1):
                        for n in range(1, numharm+1):
                                P_ms_nh = COMP_period_ms * (np.float64(n) / nh)

                                if (P_ms_nh >= REF_period_low_ms) and (P_ms_nh <= REF_period_high_ms):
...
def remove_harmonics(REF_period_ms, periods_list_ms, REF_flag, Flag_cands, Name_cands, numharm, Curr_num, Curr_den, fourier_bin, BIN_FACTOR, known_freq_low, known_freq_high,REF_idx):
        ...
        for cand_idx in range(len(periods_list_ms)):
                ...
                if Flag_cands[cand_idx] == -1:
                        ...
                        if (periods_list_ms[cand_idx] > REF_period_low_ms) and (periods_list_ms[cand_idx] < REF_period_high_ms):
                               ...
                               print("Candidate %d with period %.4f ms removed because in the same interval of candidate %d with period %.4f ms" % (cand_idx,periods_list_ms[cand_idx],REF_idx,REF_period_ms))
                                ...
                                for nh in range(1, numharm + 1):
                                        for n in range(1, numharm+1):
                                                P_ms_nh = periods_list_ms[cand_idx] * (np.float64(n) / nh)
...
def import_known_pulsars(PAR_FOLD, Tstart_MJD, Fourier_bin):
        ################################################################################
        ...
        if os.path.exists(PAR_FOLD):
                ...
                for k in range(len(list_parfilenames)):
                        ...
                        current_freq_low_lim = (1/current_p_long) - Fourier_bin # LOWER FREQUENCY DUE TO DOPPLER AND ADDING FOURIER BIN UNCERTAINTY
                        current_freq_high_lim = (1/current_p_short) + Fourier_bin # HIGHER FREQUENCY DUE TO DOPPLER AND ADDING FOURIER BIN UNCERTAINTY
...
def prepfold_command_from_csv(Results_dir,root_dir,Longest_file,n_cpu=None):
        ...
        Curr_periods_ms = np.array(Curr_result_file.Period_ms).astype("float")
        Curr_maxdms = np.array(Curr_result_file.MaxDMs).astype("float")
